...
void selectSemWhere()
{
    ...
    {
        printf("Falha no processamento do arquivo.\n");
        liberaMemSelect(0, tipoArquivo, arquivoEntrada, fptr, cab);
        return;
    }
    ...
    {
        printf("Falha no processamento do arquivo.\n");
        liberaMemSelect(1, tipoArquivo, arquivoEntrada, fptr, cab);
        return;
    }
...
void insertInto(){
    ...
    FILE *fptrIndex = fopen(arquivoIndice, "w+b");
    ...
    for(int i =0; i < n; i++){
        registro_t *reg = lerRegistroStdin(tArquivo);
        insereRegistroNoArquivo(reg,cab,fptr,tArquivo);
    }

    if(tArquivo == 1){
        atualizaCabecariot1(cab,fptr);
    }else{
        atualizaCabecariot2(cab,fptr);
    }
